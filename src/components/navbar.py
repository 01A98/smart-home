...
class Navbar(nav):
    ...
    def __init__(
        ...
        self["class"] = (
            "block w-full max-w-screen-xl px-6 py-3 mx-auto text-white bg-white border shadow-md "
        ...
        with self:
            with div(class_name="flex items-center justify-between text-blue-gray-900"):
                ...
                with div(class_name="hidden md:block"):
                    with ul(
                        class_name="flex gap-2 my-2 md:mb-0 md:mt-0 md:flex-row md:items-center md:gap-6"
                    ):
    ...
    def navbar_page_link(self, page: Page) -> html_tag:
        with li(
            class_name="block p-1 font-sans text-sm antialiased font-medium leading-normal text-blue-gray-900"
        ):
            return a(
                page.title,
                pageName=page.name,
